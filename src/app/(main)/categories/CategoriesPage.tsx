  ...
  const CategoryCard = ({ category }: any) => {
    ...
    return (
      <Link href={`/products?category=${categorySlug}`} className="group">
        <Card className="p-6 flex flex-col items-start justify-start gap-3 text-start
      bg-gray-300 dark:bg-gray-900 border-2 border-transparent hover:border-primary/50
      transition-all duration-300 min-h-[230px]">
          {showImage && iconUrl ? (
            <div className="w-24 h-24 rounded-xl overflow-hidden border-2 border-gray-500 shadow-lg flex-shrink-0">
              <img
                src={iconUrl}
                alt={category.name}
            ...
            <IconComponent className="h-24 w-24 text-white bg-gray-500 border shadow rounded-xl
          p-4 transition-transform flex-shrink-0" />
          ) : (
            <Icons.Component className="h-24 w-24 text-white bg-gray-500 border shadow rounded-xl
          p-4 transition-transform flex-shrink-0" />
          )}
          <h3 className="mt-4 text-md font-semibold text-start">{category.name}</h3>
        </Card>
      </Link>
    );
  };
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        <header className="mb-12">
          ...
          <p className="mt-3 max-w-4xl text-lg text-muted-foreground">
            Our marketplace is organized to make discovery simple, fast, and scalable. Instead of overwhelming users with unstructured listings, we categorize AI tools and digital products based on real business needs and practical use cases.
          </p>
        </header>
        ...
        <div className={`
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-lg  text-muted-foreground mb-8 max-w-3xl mx-auto">
              The AI landscape evolves rapidly. New prompts, automation workflows, digital templates, and SaaS solutions are added regularly to keep pace with innovation.            </p>
